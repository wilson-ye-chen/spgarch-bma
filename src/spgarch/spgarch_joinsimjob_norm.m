function [iSamp, M, L, U, x, Y, Pk, Sig, acc] = ...
    spgarch_joinsimjob_norm(dirName)
% [iSamp, M, L, U, x, Y, Pk, Sig, acc] = spgarch_joinsimjob_norm(dirName)
% joins the result files in a single directory generated by the simulation
% jobs. The directory must contain only the result files.
%
% Input:
% dirName - directory name.
%
% Output:
% iSamp   - sample index.
% M       - matrix of posterior means of mu and omega.
% L       - matrix of the lower-bound of the 95-percent posterior interval.
% U       - matrix of the upper-bound of the 95-percent posterior interval.
% x       - vector of points at which the coefficient function is evaluated.
% Y       - matrix of function values of the coefficient function.
% Pk      - matrix of marginal posterior probabilities of each knot.
% Sig     - matrix of conditional volatilities.
% acc     - vector of acceptance rates.
%
% Author: Wilson Ye Chen <yche5077@uni.sydney.edu.au>
% Date:   March 9, 2016

    nDim = 2;
    nKnot = 9;
    nObs = 4001;
    
    x = -4:0.01:4;
    nX = numel(x);
    
    file = dir(dirName);
    nFile = numel(file) - 2;
    
    iSamp = zeros(nFile, 1);
    M = zeros(nFile, nDim);
    L = zeros(nFile, nDim);
    U = zeros(nFile, nDim);
    Y = zeros(nFile, nX);
    Pk = zeros(nFile, nKnot);
    Sig = zeros(nFile, nObs);
    acc = zeros(nFile, 1);
    
    for i = 1:nFile
        load([dirName, '/', file(i + 2).name]);
        iSamp(i) = iSet;
        M(i, :) = mean(Chain(:, (1:nDim)), 1);
        L(i, :) = quantile(Chain(:, (1:nDim)), 0.025, 1);
        U(i, :) = quantile(Chain(:, (1:nDim)), 0.975, 1);
        Y(i, :) = YSpl;
        Pk(i, :) = sum(Model(:, (nDim + 3 + 1):end)) ./ size(Model, 1);
        Sig(i, :) = Sigma;
        acc(i, :) = accRate;
        disp([file(i + 2).name, ' appended.']);
    end
    
    [iSamp, iSort] = sort(iSamp, 'ascend');
    M = M(iSort, :);
    L = L(iSort, :);
    U = U(iSort, :);
    Y = Y(iSort, :);
    Pk = Pk(iSort, :);
    Sig = Sig(iSort, :);
    acc = acc(iSort);
end
