function [dic, dAve, d, pD] = gjr_dic_t(Theta, r, sigmaSq0)
% [dic, dAve, d, pD] = gjr_dic_t(Theta, r, sigmaSq0) computes the DIC from
% MCMC outputs for the GJR-GARCH-t model.
%
% Input:
...
% Date:   November 8, 2016
    ...
    for i = 1:nTheta
        ...
        alpha2 = Theta(i, 5);
        beta = Theta(i, 6);
        ll(i) = gjr_like_t(r, sigmaSq0, nu, mu, omega, alpha1, alpha2, beta);
    end
    dAve = mean(-2 .* ll);
    ...
    alpha2 = thetaAve(5);
    beta = thetaAve(6);
    ll = gjr_like_t(r, sigmaSq0, nu, mu, omega, alpha1, alpha2, beta);
    d = -2 .* ll;
